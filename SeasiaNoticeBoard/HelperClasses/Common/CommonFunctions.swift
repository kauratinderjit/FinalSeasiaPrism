...
class CommonFunctions: BaseUIViewController
{
    ...
    func convertStringIntoDateWithDDMMYYYY(Str: String) -> Date{
         let dateFormatter = DateFormatter()
         // initially set the format based on your datepicker date / server String
...
//        dateFormatter.locale = Locale.current
       ...
       let date = dateFormatter.date(from: Str)
        if date != nil
        {
         ...
         if strDate != "" {
            let strDate = dateFormatter.date(from: strDate) ?? Date()
             return strDate
         }
         return Date()
     }
    ...
    func setRootLogin(){
        ...
        let vc = mainStoryBoard.instantiateViewController(withIdentifier: "nav2")
