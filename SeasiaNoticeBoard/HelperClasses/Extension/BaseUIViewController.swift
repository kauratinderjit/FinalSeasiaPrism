...
var strTitle = "Select Item for Seasia Prism"
...
class BaseUIViewController: UIViewController{
    ...
    public var imagePickerDelegate:UIImagePickerDelegate?
    ...
    func imageWithImage(image:UIImage, scaledToSize newSize:CGSize ) -> UIImage {
           ...
           image.draw(in: CGRect(x: 0, y: 20, width: newSize.width, height: newSize.width))
    ...
    func imageWithVideo(image:UIImage, scaledToSize newSize:CGSize ) -> UIImage {
    ...
    func addTopBorder(with color: UIColor?, view: UIView) {
        let border = UIView()
        border.backgroundColor = color
        border.autoresizingMask = [.flexibleWidth, .flexibleBottomMargin]
        border.frame = CGRect(x: 0, y: view.frame.origin.y +  view.frame.size.height, width: view.frame.size.width, height: 1)
...
self.datePickerView.maximumDate = Calendar.current.date(byAdding: .year, value: 0, to: Date())
    ...
    func showDatePickerDB(datePickerDelegate: SharedUIDatePickerDelegate, date: NSDate){
        view.endEditing(true)
        ...
        if(pickerType == .date){
            self.datePickerView.setDate(date as Date, animated: true)
        }else{
            self.datePickerView.setDate(date as Date, animated: true)
        }
    ...
    func initializeGalleryAlert(_ view: UIView,isHideBlurView : Bool){
        DispatchQueue.main.async {
            ...
            self.galleryAlertView.btnCamera.backgroundColor = UIColor.init(red: 24/255, green: 174/255, blue: 122/255, alpha: 1)
            self.galleryAlertView.btnCancel.backgroundColor = UIColor.init(red: 24/255, green: 174/255, blue: 122/255, alpha: 1)
            self.galleryAlertView.btnGallery.backgroundColor = UIColor.init(red: 24/255, green: 174/255, blue: 122/255, alpha: 1)
            self.galleryAlertView.lblAlertTitle.backgroundColor = UIColor.init(red: 24/255, green: 174/255, blue: 122/255, alpha: 1)
    ...
    func initializeCustomOkAlert(_ view: UIView,isHideBlurView : Bool){
        DispatchQueue.main.async {
            ...
            self.okAlertView.btnOK.backgroundColor = UIColor.init(red: 24/255, green: 174/255, blue: 122/255, alpha: 1)
            self.okAlertView.lblAlertTitle.backgroundColor = UIColor.init(red: 24/255, green: 174/255, blue: 122/255, alpha: 1)
    ...
    func initializeCustomYesNoAlert(_ view: UIView,isHideBlurView : Bool){
        DispatchQueue.main.async {
            self.view.endEditing(true)
           // self.yesNoAlertView.imgLogo.setImageColor(color: UIColor.init(red: 24/255, green: 174/255, blue: 122/255, alpha: 1))
            self.yesNoAlertView.lblAlertTitle.backgroundColor = UIColor.white
                self.yesNoAlertView.noBtn.backgroundColor = UIColor.init(red: 24/255, green: 174/255, blue: 122/255, alpha: 1)
                self.yesNoAlertView.yesBtn.backgroundColor = UIColor.init(red: 24/255, green: 174/255, blue: 122/255, alpha: 1)
    ...
    func initializeCustomTextFieldView(_ view: UIView,isHideBlurView : Bool){
        DispatchQueue.main.async {
...
//        }
    ...
    func OpenGalleryCamera(camera:Bool,imagePickerDelegate:UIImagePickerDelegate){
        if(camera == true){
            ...
            self.imagePickerDelegate = imagePickerDelegate
            imagePicker.delegate = self
            strTitle = "Select Photo for Seasia Prism"
            self.present(imagePicker, animated: true, completion: nil)
        }
    }
    ...
    func OpenGalleryVideo(camera:Bool,imagePickerDelegate:UIImagePickerDelegate) {
               ...
               self.imagePickerDelegate = imagePickerDelegate
             imagePicker.navigationItem.title = "Video"
               strTitle = "Select Video for Seasia Prism"
               self.present(imagePicker, animated: true, completion: nil)
    ...
    func removeBlurEffect() {
      ...
      func openCropper(with figure: ImageCropperConfiguration.ImageCropperFigureType, image: Data, cornerRadius: CGFloat?) {
        guard let img = UIImage(data: image) else { return }
          var config = ImageCropperConfiguration(with: img, and: ImageCropperConfiguration.ImageCropperFigureType.rect4x3, cornerRadius: cornerRadius)
        ...
        let cropper = ImageCropperViewController.initialize(with: config, completionHandler: { _croppedImage in
          ...
          croppedImage = _img
            boolSelectedImg = true
            self.openPostProduction(with: croppedImage)

        }) {
            if boolSelectedImg == false {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]){
        ...
        if(isImage == true){
            if var pickedImage = info[UIImagePickerController.InfoKey.originalImage] as? UIImage{
                ...
                pickedImage = resizeImage(image: pickedImage, targetSize: CGSize(width: 400, height: 400))
                
                if #available(iOS 11.0, *) {
                   
                    if imageGarrFromAddFeed ==  true {
                        boolImageSelected = true
                    guard let imageData = pickedImage.pngData() else {
                       return
                            }
                        guard let figure = ImageCropperConfiguration.ImageCropperFigureType(rawValue: 4) else { return }
                                     openCropper(with: figure, image: imageData, cornerRadius: nil)
                    ...
                    if (info[UIImagePickerController.InfoKey.originalImage] as? UIImage) != nil {
                        ...
                        if let data = chosenImage.jpegData(compressionQuality: 0.4),
                            !FileManager.default.fileExists(atPath: fileURL.path) {
                            do {
                                // writes the image data to disk
                                try data.write(to: fileURL)
            ...
            do {
                ...
                let cgImage = try imgGenerator.copyCGImage(at: CMTimeMake(value: 1, timescale: 600), actualTime: nil)
...
//        navigationController.topViewController?.title =   strTitle
...
extension BaseUIViewController : UISearchBarDelegate{
    ...
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        ...
        self.navigationSearchBarDelegate?.textDidChange(searchBar: searchBar, searchText: searchBar.text ?? "")
