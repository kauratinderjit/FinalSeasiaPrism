...
class NewsLetterAndFeedVC: BaseUIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
       ...
       // DispatchQueue.main.async {
           ...
           button = UIImageView(frame: CGRect(x: 0,y: 0,width: 40, height: 40))
           clickbutton.addTarget(self, action: #selector(buttonAction), for: .touchUpInside)
            cornerView(radius: 20, view: button)
...
//               }
    ...
    func setProfileImage() {
        self.button.image = UIImage(named: "profileImage")
                ...
                if  AppDefaults.shared.userImage != ""
                                        {
                                            var image = UIImage()
                                             do {
                                             image = UIImage(data: try Data(contentsOf: URL(string: AppDefaults.shared.userImage)!))
                                                ?? UIImage(named: "profileImage")!                                    }
                                            catch {
                                            }
                                          
                                             DispatchQueue.main.async {
        //                                    image = self.resizeImage(image: image ?? UIImage(named: "profileImage")!, targetSize: CGSize(width: 60, height: 60))
                                                let thumb1 = image.resized(toWidth: 100.0)
